...
func convertToSourceReferenceArray(field interface{}) []models.SourceReference {
	...
	case []interface{}:
		for _, item := range v {
			...
			case string:
				// Assumes the string is a source ID in hex format
				if oid, err := primitive.ObjectIDFromHex(itemType); err == nil {
					ref.SourceID = oid
				}
			case map[string]interface{}:
				// Handles structured source references
				// Handle BSON ObjectID format
				if idMap, ok := itemType["$oid"].(string); ok {
					if oid, err := primitive.ObjectIDFromHex(idMap); err == nil {
						ref.SourceID = oid
					}
				} else if id, ok := itemType["source_id"].(string); ok {
				...
				if pageNums, ok := itemType["page_numbers"].(string); ok {
					ref.PageNumbers = pageNums
				}
				if chapterNum, ok := itemType["chapter_number"].(string); ok {
					ref.ChapterNumber = chapterNum
				}
				if chapterTitle, ok := itemType["chapter_title"].(string); ok {
					ref.ChapterTitle = chapterTitle
				}
			}
			if ref.SourceID != primitive.NilObjectID {
	...
	case []string:
		for _, str := range v {
			if oid, err := primitive.ObjectIDFromHex(str); err == nil {
				result = append(result, models.SourceReference{SourceID: oid})
			}
		}
	}
